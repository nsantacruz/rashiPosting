...
parsed_root = "C:\\Users\\nss\\Documents\\Github\\Sefaria-Data\\sources\\Noah-Santacruz-rashiPosting"
        ...
        for prop in dhmObj:
                if prop == 'dhm':
                        ...
                        if len(dhmObj[prop]) > 1 or len(dhmObj[prop]) == 0:
                                ...
                                print("not posted because location was ambiguous")
                                return
                        line = dhmObj[prop][0]
                elif prop == 'numOnLine':
...
def post_amud(mesechta,daf,engComm):
        ...
        print(mesechta + " " + daf)
        numPosted = 0
        text = [[] for i in range(len(dafLines))]    #initialize an array of arrays of length number of lines in daf 
        for dhmObj in dafDHms:
                ...
                if len(dhmObj['postable_lines']) > 1 or len(dhmObj['postable_lines']) == 0:
                        ...
                        print("not posted because location was ambiguous")
                        continue
                line = dhmObj['postable_lines'][0]
                text[line-1].append(dhmObj['text'])
                numPosted += 1
...
def get_index(mesechta,daf,line,withCommentary=False):
        ...
        if withCommentary:
                url = sefaria_root + '/api/texts/' + mesechta + '.' + daf + '.' + line + '?context=0&count_after=0'
        else:
                url = sefaria_root + '/api/texts/' + mesechta + '.' + daf + '.' + line + '?context=0&commentary=0&count_after=0'
        try :
...
def push_commentary(mesechta,daf,amud,commentary):
        global dafDHms
        engComm = commentary_translator(commentary)
        engMesechta = mesechta_translator(mesechta.replace('_',' ')).replace(' ','_')
...
def push_mesechta(mesechta,commentary):
        ...
        i = 0
        hebMes = mesechta_translator(mesechta,False).replace(' ','_')
        hebCommentary = commentary_translator(commentary)
        print(hebMes.encode('utf-8'))
        for filename in os.listdir(parsed_root + '\\' + commentary): #+ '\\' + 'test'):
